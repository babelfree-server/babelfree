<?php
/**
 * seed-dict-a2-curated.php — Seed curated A2 dictionary entries
 *
 * Loads dict_entries_a2.json and upserts:
 *   - dict_words (Spanish entries with POS, gender, IPA, CEFR=A2)
 *   - dict_definitions (Spanish definition per word)
 *   - dict_examples (context phrase per word)
 *
 * Run: php seed-dict-a2-curated.php [--dry-run]
 *
...
echo "=== A2 Curated Dictionary Seeder ===\n";
...
$file = "$dataDir/dict_entries_a2.json";
if (!file_exists($file)) {
...
echo "  Loaded " . count($entries) . " entries from dict_entries_a2.json\n\n";
...
$stmtWord = $pdo->prepare("
    ...
    VALUES ('es', :word, :norm, :pos, :gender, :ipa, 'A2')
    ON DUPLICATE KEY UPDATE
        ...
        cefr_level = CASE WHEN cefr_level = 'A1' THEN 'A1' ELSE 'A2' END
");
...
$stmtExCheck = $pdo->prepare("
    SELECT id FROM dict_examples WHERE word_id = :wid AND source = 'curated_a2' LIMIT 1
");
$stmtExInsert = $pdo->prepare("
    INSERT INTO dict_examples (word_id, sentence, cefr_level, source)
    VALUES (:wid, :sentence, 'A2', 'curated_a2')
");
