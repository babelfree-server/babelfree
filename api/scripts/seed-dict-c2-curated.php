<?php
/**
 * seed-dict-c2-curated.php — Seed curated C2 dictionary entries
 *
 * Loads dict_entries_c2.json and upserts:
 *   - dict_words (Spanish entries with POS, gender, IPA, CEFR=C2)
 *   - dict_definitions (Spanish definition per word)
 *   - dict_examples (context phrase per word)
 *
 * Run: php seed-dict-c2-curated.php [--dry-run]
 *
...
echo "=== C2 Curated Dictionary Seeder ===\n";
...
$file = "$dataDir/dict_entries_c2.json";
if (!file_exists($file)) {
...
echo "  Loaded " . count($entries) . " entries from dict_entries_c2.json\n\n";
...
$stmtWord = $pdo->prepare("
    ...
    VALUES ('es', :word, :norm, :pos, :gender, :ipa, 'C2')
    ON DUPLICATE KEY UPDATE
        ...
        cefr_level = CASE WHEN cefr_level IN ('A1','A2','B1','B2','C1') THEN cefr_level ELSE 'C2' END
");
...
$stmtExCheck = $pdo->prepare("
    SELECT id FROM dict_examples WHERE word_id = :wid AND source = 'curated_c2' LIMIT 1
");
$stmtExInsert = $pdo->prepare("
    INSERT INTO dict_examples (word_id, sentence, cefr_level, source)
    VALUES (:wid, :sentence, 'C2', 'curated_c2')
");
...
try {
    foreach ($entries as $i => $e) {
        ...
        $stmtWord->execute([':word' => $word, ':norm' => $norm, ':pos' => $pos, ':gender' => $gender, ':ipa' => $ipa]);
        $stats['words_upserted']++;
