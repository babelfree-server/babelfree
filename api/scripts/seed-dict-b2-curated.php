<?php
/**
 * seed-dict-b2-curated.php — Seed curated B2 dictionary entries
 *
 * Loads dict_entries_b2.json and upserts:
 *   - dict_words (Spanish entries with POS, gender, IPA, CEFR=B2)
 *   - dict_definitions (Spanish definition per word)
 *   - dict_examples (context phrase per word)
 *
 * Run: php seed-dict-b2-curated.php [--dry-run]
 *
...
echo "=== B2 Curated Dictionary Seeder ===\n";
...
$file = "$dataDir/dict_entries_b2.json";
if (!file_exists($file)) {
...
echo "  Loaded " . count($entries) . " entries from dict_entries_b2.json\n\n";
...
$stmtWord = $pdo->prepare("
    ...
    VALUES ('es', :word, :norm, :pos, :gender, :ipa, 'B2')
    ON DUPLICATE KEY UPDATE
        ...
        cefr_level = CASE WHEN cefr_level IN ('A1','A2','B1') THEN cefr_level ELSE 'B2' END
");
...
$stmtExCheck = $pdo->prepare("
    SELECT id FROM dict_examples WHERE word_id = :wid AND source = 'curated_b2' LIMIT 1
");
$stmtExInsert = $pdo->prepare("
    INSERT INTO dict_examples (word_id, sentence, cefr_level, source)
    VALUES (:wid, :sentence, 'B2', 'curated_b2')
");
