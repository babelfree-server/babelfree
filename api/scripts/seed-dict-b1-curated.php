<?php
/**
 * seed-dict-b1-curated.php — Seed curated B1 dictionary entries
 *
 * Loads dict_entries_b1.json and upserts:
 *   - dict_words (Spanish entries with POS, gender, IPA, CEFR=B1)
 *   - dict_definitions (Spanish definition per word)
 *   - dict_examples (context phrase per word)
 *
 * Run: php seed-dict-b1-curated.php [--dry-run]
 *
...
echo "=== B1 Curated Dictionary Seeder ===\n";
...
$file = "$dataDir/dict_entries_b1.json";
if (!file_exists($file)) {
...
echo "  Loaded " . count($entries) . " entries from dict_entries_b1.json\n\n";
...
$stmtWord = $pdo->prepare("
    ...
    VALUES ('es', :word, :norm, :pos, :gender, :ipa, 'B1')
    ON DUPLICATE KEY UPDATE
        ...
        cefr_level = CASE WHEN cefr_level IN ('A1','A2') THEN cefr_level ELSE 'B1' END
");
...
$stmtExCheck = $pdo->prepare("
    SELECT id FROM dict_examples WHERE word_id = :wid AND source = 'curated_b1' LIMIT 1
");
$stmtExInsert = $pdo->prepare("
    INSERT INTO dict_examples (word_id, sentence, cefr_level, source)
    VALUES (:wid, :sentence, 'B1', 'curated_b1')
");
