<?php
/**
 * seed-dict-c1-curated.php — Seed curated C1 dictionary entries
 *
 * Loads dict_entries_c1.json and upserts:
 *   - dict_words (Spanish entries with POS, gender, IPA, CEFR=C1)
 *   - dict_definitions (Spanish definition per word)
 *   - dict_examples (context phrase per word)
 *
 * Run: php seed-dict-c1-curated.php [--dry-run]
 *
...
echo "=== C1 Curated Dictionary Seeder ===\n";
...
$file = "$dataDir/dict_entries_c1.json";
if (!file_exists($file)) {
...
echo "  Loaded " . count($entries) . " entries from dict_entries_c1.json\n\n";
...
$stmtWord = $pdo->prepare("
    ...
    VALUES ('es', :word, :norm, :pos, :gender, :ipa, 'C1')
    ON DUPLICATE KEY UPDATE
        ...
        cefr_level = CASE WHEN cefr_level IN ('A1','A2','B1','B2') THEN cefr_level ELSE 'C1' END
");
...
$stmtExCheck = $pdo->prepare("
    SELECT id FROM dict_examples WHERE word_id = :wid AND source = 'curated_c1' LIMIT 1
");
$stmtExInsert = $pdo->prepare("
    INSERT INTO dict_examples (word_id, sentence, cefr_level, source)
    VALUES (:wid, :sentence, 'C1', 'curated_c1')
");
